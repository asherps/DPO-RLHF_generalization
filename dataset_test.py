...
reward_model = AutoModelForSequenceClassification.from_pretrained(
    reward_model_path, torch_dtype=t.bfloat16
).eval()
...
# Load the dataset with the correct preprocessing
dataset = utils.load_dataset(tokenizer, "Anthropic/hh-rlhf", "default", debug=False)
test_dataset = dataset["test"][:100]


# Function to prepare data for the reward model
...
with t.no_grad():
    for i in range(len(test_dataset)):
        chosen_rejected_list = []
        for chosen, rejected in zip(
            dataset["train"]["chosen"][i], dataset["train"]["rejected"][i]
        ):

            output = reward_model(**test_dataset[i])
            logits = output.logits
            chosen_rejected_list.append(logits[:, 0])
            logits_list.append(logits[:, 0])  # Extract the first logit
        num_correct = (chosen_rejected_list[0] > chosen_rejected_list[1]).float()
        num_total += 1
        print(num_correct)
        print(num_total)
# Optionally, print or process the logits_list as needed
print(logits_list)


# dataset_info = {"name": "Anthropic/hh-rlhf", "data_dir": "default"}
...
#     tokenizer, dataset_info["name"], dataset_info["data_dir"], debug=True
# )
